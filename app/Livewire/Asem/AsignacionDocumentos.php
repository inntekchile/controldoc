...
class AsignacionDocumentos extends Component
{
    ...
    // Propiedades para la asignación
    ...
    private function buildQuery()
    {
        $query = DocumentoCargado::query()
            ...
            ->where(function ($q) {
                $q->whereIn('estado_validacion', ['Pendiente', 'Rechazado'])
                  ->orWhere('requiere_revalidacion', true);
            });
        ...
        if (!empty($this->filtroEstado)) { $query->where(function ($q) { switch ($this->filtroEstado) { case 'Sin Asignar': $q->whereNull('asem_validador_id')->where('requiere_revalidacion', false)->where('estado_validacion', 'Pendiente'); break; case 'Asignado': $q->whereNotNull('asem_validador_id')->where('requiere_revalidacion', false); break; case 'Revalidar': $q->where('requiere_revalidacion', true); break; case 'Devuelto': $q->whereNull('asem_validador_id')->where('estado_validacion', 'Rechazado')->where('requiere_revalidacion', false); break; } }); }
