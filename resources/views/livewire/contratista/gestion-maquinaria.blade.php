<div>
    ...
    {{-- VISTA: LISTADO DE MAQUINARIA --}}
    @if ($vistaActual === 'listado_maquinaria')
        @if ($unidadOrganizacionalId)
            <div class="mb-4 flex flex-col sm:flex-row justify-between items-center">
                <div class="w-full sm:w-2/3 mb-2 sm:mb-0">
                    <input wire:model.live.debounce.300ms="searchMaquinaria" type="text"
                           placeholder="Buscar por Código/Patente, Marca o Tipo..." class="input-field w-full">
                </div>
                <button wire:click="abrirModalNuevaMaquinaria" class="btn-primary">
                    <x-icons.plus class="w-5 h-5 mr-1 inline-block"/> Agregar Ficha Maquinaria
                </button>
            </div>
            <div class="overflow-x-auto shadow-md sm:rounded-lg">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="table-header">Código / Patente</th> <th class="table-header">Marca / Tipo</th>
                            <th class="table-header">Año Fab.</th> <th class="table-header text-center">Estado Ficha</th>
                            <th class="table-header text-center">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($maquinariasPaginadas ?? [] as $item)
                            <tr class="table-row-hover">
                                <td class="table-cell font-mono">{{ $item->identificador_completo }}</td>
                                <td class="table-cell">
                                    <span class="font-semibold">{{ $item->marca->nombre ?? 'N/A' }}</span>
                                    <span class="block text-xs text-gray-500">{{ $item->tipoMaquinaria->nombre ?? 'N/A' }}</span>
                                </td>
                                <td class="table-cell">{{ $item->ano_fabricacion }}</td>
                                <td class="table-cell text-center"><span wire:click="toggleActivoMaquinaria({{ $item->id }})" class="status-badge {{ $item->is_active ? 'status-active' : 'status-inactive' }}">{{ $item->is_active ? 'Activo' : 'Inactivo' }}</span></td>
                                <td class="table-cell text-center whitespace-nowrap">
                                    <button wire:click="abrirModalEditarMaquinaria({{ $item->id }})" class="action-button-edit" title="Editar Ficha"><x-icons.edit/></button>
                                    <button wire:click="eliminarMaquinaria({{ $item->id }})" wire:confirm="¿Estás seguro?\n\nSe eliminará la ficha de la maquinaria y TODAS sus asignaciones." class="action-button-delete" title="Eliminar Ficha"><x-icons.trash/></button>
                                    <button wire:click="seleccionarMaquinariaParaAsignaciones({{ $item->id }})" class="action-button-link" title="Ver/Gestionar Asignaciones"><x-icons.link/></button>
                        ...
                        @empty
                            <tr><td colspan="5" class="table-cell text-center">No se encontraron maquinarias asignadas a esta Unidad Organizacional.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            @if($maquinariasPaginadas && $maquinariasPaginadas->hasPages())<div class="mt-4">{{ $maquinariasPaginadas->links(data: ['pageName' => 'maquinariaPage']) }}</div>@endif
        @else
    ...
    @elseif ($vistaActual === 'listado_asignaciones' && $maquinariaSeleccionada)
        <div class="mb-4 p-4 border dark:border-gray-700 rounded-md">
            <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300">Maquinaria: <span class="font-normal">{{ $maquinariaSeleccionada->identificador_completo }} ({{ $maquinariaSeleccionada->marca->nombre ?? 'N/A' }})</span></h3>
            <button wire:click="abrirModalEditarMaquinaria({{ $maquinariaSeleccionada->id }})" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Editar Ficha de esta Maquinaria</button>
        </div>
        <div class="mb-4 flex flex-col sm:flex-row justify-between items-center">
            <button wire:click="irAListadoMaquinaria" class="btn-secondary mb-2 sm:mb-0"><x-icons.arrow-left class="w-5 h-5 mr-1"/> Volver al Listado</button>
        ...
        <div class="overflow-x-auto shadow-md sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    @empty
                    <tr><td colspan="4" class="table-cell text-center">Esta maquinaria no tiene asignaciones a Unidades Organizacionales.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    @if ($showModalFicha)
         <div class="fixed z-20 inset-0 overflow-y-auto" aria-labelledby="modal-title-maquinaria" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
             ...
             <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                 <form wire:submit.prevent="guardarMaquinaria" id="formFichaMaquinaria">
                     <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4"><h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100 section-title">{{ $maquinariaId ? 'Editar Ficha de Maquinaria' : 'Agregar Nueva Ficha de Maquinaria' }}</h3><div class="mt-4 space-y-4"><div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-3 section-title !mt-0 !border-none !pb-0">Identificación</div>
                        <div class="md:col-span-2"><label class="label-form">Patente / Código <span class="text-red-500">*</span></label><div class="flex items-center space-x-2"><input type="text" wire:model.lazy="identificador_letras" id="identificador_letras" class="input-field w-1/2 uppercase" placeholder="LETRAS / CÓDIGO"><span class="text-gray-400">-</span><input type="text" wire:model.lazy="identificador_numeros" id="identificador_numeros" class="input-field w-1/2 uppercase" placeholder="NÚMEROS"></div>@error('identificador_letras') <span class="error-message">{{ $message }}</span> @enderror @error('identificador_numeros') <span class="error-message">{{ $message }}</span> @enderror</div>
                        <div><label for="ano_fabricacion_maq" class="label-form">Año de Fabricación <span class="text-red-500">*</span></label><input type="number" wire:model.lazy="ano_fabricacion" id="ano_fabricacion_maq" class="input-field w-full" placeholder="YYYY">@error('ano_fabricacion') <span class="error-message">{{ $message }}</span> @enderror</div>
                        <div class="md:col-span-3 section-title !border-none !pb-0">Características</div>
                        <div><label for="marca_vehiculo_id_maq" class="label-form">Marca <span class="text-red-500">*</span></label><select wire:model="marca_vehiculo_id" id="marca_vehiculo_id_maq" class="input-field w-full"><option value="">Seleccione...</option>@foreach($marcas as $marca) <option value="{{ $marca->id }}">{{ $marca->nombre }}</option> @endforeach</select>@error('marca_vehiculo_id') <span class="error-message">{{ $message }}</span> @enderror</div>
                        <div><label for="tipo_maquinaria_id" class="label-form">Tipo de Maquinaria <span class="text-red-500">*</span></label><select wire:model="tipo_maquinaria_id" id="tipo_maquinaria_id" class="input-field w-full"><option value="">Seleccione...</option>@foreach($tiposMaquinaria as $tipo) <option value="{{ $tipo->id }}">{{ $tipo->nombre }}</option> @endforeach</select>@error('tipo_maquinaria_id') <span class="error-message">{{ $message }}</span> @enderror</div>
                        <div><label for="tenencia_vehiculo_id_maq" class="label-form">Propiedad (Opcional)</label><select wire:model="tenencia_vehiculo_id" id="tenencia_vehiculo_id_maq" class="input-field w-full"><option value="">Seleccione...</option>@foreach($tenencias as $tenencia) <option value="{{ $tenencia->id }}">{{ $tenencia->nombre }}</option> @endforeach</select>@error('tenencia_vehiculo_id') <span class="error-message">{{ $message }}</span> @enderror</div>
                        <div class="md:col-span-3"><label for="maquinaria_is_active_modal" class="label-form flex items-center mt-2"><input type="checkbox" wire:model="maquinaria_is_active" id="maquinaria_is_active_modal" class="form-checkbox h-5 w-5"><span class="ms-2 text-sm text-gray-600 dark:text-gray-400">Ficha de Maquinaria Activa</span></label></div>
                     </div></div></div>
                     <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse"><button type="submit" class="btn-primary w-full sm:w-auto sm:ml-3">{{ $maquinariaId ? 'Guardar Cambios' : 'Crear Ficha' }}</button><button type="button" wire:click="cerrarModalFicha" class="btn-secondary w-full mt-3 sm:mt-0 sm:w-auto">Cancelar</button></div>
                 </form>
             </div>
         </div>
        </div>
    ...
    @if ($showModalAsignacion)
        <div class="fixed z-30 inset-0 overflow-y-auto"><div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0"><div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" wire:click="cerrarModalAsignacion"></div><div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form wire:submit.prevent="guardarAsignacion"><div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4"><h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 section-title">{{ $asignacionId ? 'Editar Asignación' : 'Nueva Asignación' }} para Maquinaria: {{ $maquinariaSeleccionada->identificador_completo }}</h3><div class="mt-4 space-y-4">
                <div><label for="a_unidad_organizacional_id" class="label-form">Unidad Organizacional <span class="text-red-500">*</span></label><select wire:model="a_unidad_organizacional_id" id="a_unidad_organizacional_id" class="input-field w-full"><option value="">Seleccione...</option>@foreach($unidadesOrganizacionalesDisponibles as $uo)<option value="{{ $uo->id }}">{{ $uo->mandante->razon_social }} - {{ $uo->nombre_unidad }}</option>@endforeach</select>@error('a_unidad_organizacional_id')<span class="error-message">{{$message}}</span>@enderror</div>
                <div><label for="a_fecha_asignacion" class="label-form">Fecha Asignación <span class="text-red-500">*</span></label><input type="date" wire:model.lazy="a_fecha_asignacion" id="a_fecha_asignacion" class="input-field w-full">@error('a_fecha_asignacion')<span class="error-message">{{$message}}</span>@enderror</div>
                ...
                @if(!$a_is_active)<div class="space-y-4">
                    <div><label for="a_fecha_desasignacion" class="label-form">Fecha de Desactivación <span class="text-red-500">*</span></label><input type="date" wire:model.lazy="a_fecha_desasignacion" id="a_fecha_desasignacion" class="input-field w-full">@error('a_fecha_desasignacion') <span class="error-message">{{ $message }}</span> @enderror</div>
                    <div><label for="a_motivo_desasignacion" class="label-form">Motivo de Desactivación <span class="text-red-500">*</span></label><textarea wire:model.lazy="a_motivo_desasignacion" id="a_motivo_desasignacion" rows="3" class="input-field w-full"></textarea>@error('a_motivo_desasignacion') <span class="error-message">{{ $message }}</span> @enderror</div>
                </div>@endif
            </div></div>
    ...
    {{-- MODAL PARA GESTIONAR DOCUMENTOS DE MAQUINARIA --}}
    @if ($showDocumentosModal && $maquinariaParaDocumentos)
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-7xl sm:w-full">
                    <form wire:submit.prevent="cargarDocumentos">
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6">
                            <h3 class="text-xl leading-6 font-medium text-gray-900 dark:text-gray-100 section-title mb-1">Documentos Requeridos para: <span class="font-semibold">{{ $nombreMaquinariaParaDocumentosModal }}</span></h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Maquinaria: {{ $maquinariaParaDocumentos->marca?->nombre }} {{ $maquinariaParaDocumentos->tipoMaquinaria?->nombre }}</p>
                            ...
                            <div class="mt-6 overflow-x-auto shadow-md sm:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        @empty
                                            <tr><td colspan="9" class="table-cell text-center p-4">No se encontraron documentos requeridos para esta maquinaria en esta Unidad Organizacional.</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
