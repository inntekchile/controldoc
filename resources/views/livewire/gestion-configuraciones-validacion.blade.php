<div class="p-6 bg-white dark:bg-gray-800 shadow-md rounded-lg">
    <h2 class="text-2xl font-semibold text-gray-800 dark:text-white mb-4">Gestión de Configuraciones de Validación</h2>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
        <div class="w-full sm:w-1/3">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar configuración..."
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">1er Validador</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">2do Validador</th>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($configuraciones as $item)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $item->primerRolValidador->name ?? 'N/A' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $item->segundoRolValidador->name ?? 'N/A' }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            ...
                            <button wire:click="delete({{ $item->id }})"
                                    wire:confirm="¿Está seguro de que quiere desactivar esta Configuración?"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 text-center">
                            No se encontraron configuraciones de validación.
                        </td>
                    </tr>
                @endforelse
            </tbody>
                            ...
                            <div class="sm:flex sm:items-start w-full">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">
                                        {{ $configId ? 'Editar' : 'Crear Nueva' }} Configuración de Validación
                                    </h3>
                                    <div class="mt-2">
                                        <!-- Campo Nombre -->
                                        <div class="mb-4">
                                            <label for="nombre_cv" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre <span class="text-red-500">*</span></label>
                                            <input type="text" wire:model.lazy="nombre" id="nombre_cv" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('nombre') border-red-500 @enderror">
                                            @error('nombre') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                        </div>

                                        <!-- Campo Descripción -->
                                        <div class="mb-4">
                                            <label for="descripcion_cv" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Descripción</label>
                                            <textarea wire:model.lazy="descripcion" id="descripcion_cv" rows="3" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('descripcion') border-red-500 @enderror"></textarea>
                                            @error('descripcion') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                        </div>

                                        <!-- Primer Rol Validador -->
                                        <div class="mb-4">
                                            <label for="primer_rol_validador_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Primer Rol Validador</label>
                                            <select wire:model="primer_rol_validador_id" id="primer_rol_validador_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('primer_rol_validador_id') border-red-500 @enderror">
                                                <option value="">-- Ninguno --</option>
                                                @foreach ($rolesParaSelect as $role)
                                                    <option value="{{ $role->id }}">{{ $role->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('primer_rol_validador_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                        </div>

                                        <!-- Segundo Rol Validador -->
                                        <div class="mb-4">
                                            <label for="segundo_rol_validador_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Segundo Rol Validador</label>
                                            <select wire:model="segundo_rol_validador_id" id="segundo_rol_validador_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('segundo_rol_validador_id') border-red-500 @enderror">
                                                <option value="">-- Ninguno --</option>
                                                @foreach ($rolesParaSelect as $role)
                                                    <option value="{{ $role->id }}">{{ $role->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('segundo_rol_validador_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                        </div>
                                        
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">Nota: Si se selecciona un segundo rol, el primero es obligatorio. Si solo se desea un validador, selecciónelo en "Primer Rol Validador" y deje el segundo como "Ninguno".</p>

                                        <!-- Campo Activo -->
                                        <div class="mb-4">
                                            <label for="is_active_cv_modal" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                                                <input type="checkbox" wire:model="is_active" id="is_active_cv_modal" class="h-4 w-4 text-indigo-600 border-gray-300 dark:border-gray-600 rounded focus:ring-indigo-500 dark:bg-gray-700 dark:focus:ring-indigo-600 dark:ring-offset-gray-800">
